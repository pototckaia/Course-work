...
def print_mean(y_t, y_tr, first, second):
    ...
    print("Mean squared error on train {:.5f} and test {:.5f}".format(mse_train, mse_test))
    
    mae_test = mean_absolute_error(y_t[first], y_t[second])
    mae_train = mean_absolute_error(y_tr[first], y_tr[second])
    print("Mean absolute error on train {:.5f} and test {:.5f}".format(mae_train, mae_test))
    ...
    print("Median absolute error on train {:.5f} and test {:.5f}".format(mear_train, mear_test))
    ...
    # print("Mean absolute percentage error on train {:.5f} and test {:.5f}".format(mape_train, mape_test))
